...
class IndexController extends Base {
    ...
    public  function showgbook(){
        ...
        if (empty($category)) {
            //send_http_status(404);
            $this->error('错误',U('Index/lists', array('caiid'=>11)));
            //exit;
        }
        ...
        if(!is_numeric($id)){
            $this->error('非法参数',U('Index/lists', array('caiid'=>11)));
        }
        $where['cxm'] = array('eq',$id);
        $res = M('gbook')->where($where)->find();
        ...
        if($res){
            if(!$res['status']){
             ...
             $this->error('查询失败,请检查查询码',U('Index/lists', array('caiid'=>23)));
        }

        $SEO = seo();
        $this->assign("SEO", $SEO);
        $this->assign($category);
        $this->display("Show/gbook");
    }
