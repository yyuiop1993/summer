...
class Content {
    ...
    public function where($attr) {
        ...
        if (isset($attr['catid']) && (int) $attr['catid']) {
            $catid=explode(',',$attr['catid']);//流浪优化新增 支持调用多栏目
			
			//循环处理多栏目
			foreach($catid as $k=>$v){
				//本栏目是父栏目，合并子栏目ID数组
				if(getCategory($v, 'child')){
					$catids_str = getCategory($v, 'arrchildid');
					$catid_arr	=explode(',',$catids_str);
					foreach($catid_arr as $sid){
						$catid_toal[]=$sid;
					}
				}else{
					$catid_toal[]=$v;//追加单个ID
			...
			$where['catid'] = array("IN", $catid_toal);
        }
    ...
    public function category($data) {
		...
		if(!strpos(getCategory($data['catid'],'arrchildid'),',')){
            $data['catid'] = getCategory($data['catid'],'parentid') ;
        }
        $where = $array = array();
        //设置SQL where 部分
        if (isset($data['where']) && $data['where']) {
