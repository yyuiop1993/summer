...
class ContentController extends AdminBase {
    ...
    public function classlist() {
        ...
        if($this->catid == 0){
             ...
             $modelid = I('get.mid',1,intval); // 定义模型实例
        }else{
        ...
        if(!$isAdministrator){
            $where['uid'] = array('EQ',User::getInstance()->id);
        }
        //信息总数
        if (empty($search)) {
            $count = $sum;
        ...
        $this->assign($catInfo)
            ...
            ->assign('count', $count)
            ->assign('data', $data)
            ->assign('isAdministrator',$isAdministrator);
        if($this->catid == 0){
             $this->display('classlist_sh');       
        }else{
    ...
    public function add() {
        ...
        if (IS_POST) {
            ...
            if ($category['type'] == 0) {
                ...
                $data = $_POST['info'];
                $data['uid'] =   User::getInstance()->id;
                $status = $this->Content->data($data)->add();
                if ($status) {
                    $this->success("添加成功！");
                } else {
    ...
    public function edit() {
        ...
        if (IS_POST) {
            ...
            $data = $_POST['info'];
            if( ($data['uid'] != User::getInstance()->id)){
                if(!$isAdministrator){
                     $this->error('更新错误');
                 }
            ...
            unset($data['info']);
            $status = $this->Content->data($data)->edit();
            if ($status) {
                //解除信息锁定
    ...
    public function tongji (){
        ...
        if(IS_POST){
            ...
            if (!empty($start_time)) {
                $start_time = strtotime($start_time);
                $jiuqu['inputtime'] = array("EGT", $start_time);
                $xianggong['inputtime'] = array("EGT", $start_time);
                $tangtou['inputtime'] = array("EGT", $start_time);
                $taiping['inputtime'] = array("EGT", $start_time);
                $zhengwang['inputtime'] = array("EGT", $start_time);
                $bahu['inputtime'] = array("EGT", $start_time);
                $tanghe['inputtime'] = array("EGT", $start_time);
                $fhl['inputtime'] = array("EGT", $start_time);
            }

            $end_time = I('end_time');
            if (!empty($end_time)) {
                $end_time = strtotime($end_time);
                $jiuqu['inputtime'] = array("ELT", $start_time);
                $xianggong['inputtime'] = array("ELT", $start_time);
                $tangtou['inputtime'] = array("ELT", $start_time);
                $taiping['inputtime'] = array("ELT", $start_time);
                $zhengwang['inputtime'] = array("ELT", $start_time);
                $bahu['inputtime'] = array("ELT", $start_time);
                $tanghe['inputtime'] = array("ELT", $start_time);
                $fhl['inputtime'] = array("ELT", $start_time);
                
            }

            if ($end_time > 0 && $start_time > 0) {                
                $jiuqu['inputtime'] = array(array('EGT', $start_time), array('ELT', $end_time));
                $xianggong['inputtime'] = array(array('EGT', $start_time), array('ELT', $end_time));
                $tangtou['inputtime'] = array(array('EGT', $start_time), array('ELT', $end_time));
                $taiping['inputtime'] = array(array('EGT', $start_time), array('ELT', $end_time));
                $zhengwang['inputtime'] = array(array('EGT', $start_time), array('ELT', $end_time));
                $bahu['inputtime'] = array(array('EGT', $start_time), array('ELT', $end_time));
                $tanghe['inputtime'] = array(array('EGT', $start_time), array('ELT', $end_time));
                $fhl['inputtime'] = array(array('EGT', $start_time), array('ELT', $end_time));

            }
                        
            $jiuqu_num = $db->where($jiuqu)->count();
            $xianggong_num = $db->where($xianggong)->count();
            $tangtou_num = $db->where($tangtou)->count();
            $taiping_num = $db->where($taiping)->count();
            $zhengwang_num = $db->where($zhengwang)->count();
            $bahu_num = $db->where($bahu)->count();
            $tanghe_num = $db->where($tanghe)->count();
            $fhl_num = $db->where($fhl)->count();
        ...
        $this->assign('jiuqu_num',$jiuqu_num);
        $this->assign('fhl_num',$fhl_num);
        $this->assign('xianggong_num',$xianggong_num);
        $this->assign('tangtou_num',$tangtou_num);
        $this->assign('taiping_num',$taiping_num );
        $this->assign('zhengwang_num',$zhengwang_num);
        $this->assign('bahu_num',$bahu_num);
        $this->assign('tanghe_num', $tanghe_num);
        $this->display();
    }
}
